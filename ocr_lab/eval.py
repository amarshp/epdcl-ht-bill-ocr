...
from common import DEV_BILLS, TEST_BILLS, classify, ocr
...
def _provenance_ok(fp):
    ...
    return (
        bool(fp)
        and fp.get("observed") is True
        and fp.get("source_boxes")
...
def main():
    ...
    pages = {"dev": DEV_BILLS, "test": TEST_BILLS, "full": list(range(1, 263))}[a.split]
    ...
    if a.gt or a.all:
        ...
        cats = [
            c for c in ("money", "consumption", "dates", "ids", "other") if agg[c][1]
        ]
        for c in cats:
            h, t = agg[c]
            print(f"  {c:12}: {h}/{t}" + (f" = {h/t:.0%}" if t else ""))
        tot_h = sum(agg[c][0] for c in cats)
        tot_t = sum(agg[c][1] for c in cats)
        print(
            f"  {'OVERALL':12}: {tot_h}/{tot_t}"
            + (f" = {tot_h/tot_t:.1%}" if tot_t else "")
        )
        print(
            "  per-page:",
            [(p, k, f"{s[0]}/{s[1]}" if s else "-") for p, d, k, s in per_page],
        )
