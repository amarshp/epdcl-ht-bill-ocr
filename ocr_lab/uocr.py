...
"""
import os, io, re, sys, contextlib, functools
import torch
...
def _load():
    ...
    if _model is None:
        ...
        _model = AutoModel.from_pretrained(_MID, trust_remote_code=True,
                                           use_safetensors=True, dtype=torch.bfloat16).eval()
    return _model, _tok
...
def transcribe(img):
    ...
    model, tok = _load()
    p = os.path.join(_TMP, "_in.png"); img.save(p)
    buf = io.StringIO()
    with contextlib.redirect_stdout(buf):
        model.infer(tok, prompt="<image>OCR.", image_file=p, output_path=os.path.join(_TMP, "out"),
...
def parse_fields(html):
    ...
    f["arrears_curr"]     = _n(re.search(r"Current Years?\)?\s*Arrears[^0-9-]*" + _NUM, t, re.I))
