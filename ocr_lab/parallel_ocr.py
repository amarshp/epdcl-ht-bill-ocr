...
if __name__ == "__main__":
    ...
    print(
        f"{len(pages)} pages to OCR | {N_WORKERS} workers x {THREADS} threads",
        flush=True,
    )
    t0 = time.time()
    done = 0
    with mp.Pool(N_WORKERS, initializer=_init) as pool:
        for p, n, dt in pool.imap_unordered(_work, pages):
            ...
            if done <= 6 or done % 20 == 0:
                ...
                print(
                    f"  [{done}/{len(pages)}] p{p}: {n} boxes in {dt}s "
                    f"| throughput {rate:.1f} pages/min",
                    flush=True,
                )
    el = time.time() - t0
    print(
        f"done: {len(pages)} pages in {el:.0f}s = {len(pages)/el*60:.1f} pages/min",
        flush=True,
    )
