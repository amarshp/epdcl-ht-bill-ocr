...
def is_sparse(boxes):
    ...
    return ("electricitybillservice" in blob or "electricitybillserviceno" in blob) \
        and "subtotal" not in blob

def _prov(b, field, rule):
    v, _ = parse_num(b[3])
    return {"value": v, "raw_text": b[3], "bbox": [b[0], b[1], b[2]],
            "source_boxes": [list(b)], "confidence": None, "rule": rule,
...
def sparse_parse(boxes):
    ...
    for row in rows(boxes, band=18):
        ...
        label = norm("".join(b[3] for b in row if parse_num(b[3])[0] is None or b[1] < 700))
    ...
    if hdr:
        ...
        if m:
            out["service_no"] = {"value": m.group(1), "raw_text": hdr[3],
                                 "bbox": [hdr[0], hdr[1], hdr[2]], "source_boxes": [list(hdr)],
                                 "confidence": None, "rule": "sparse:service_no",
                                 "observed": True, "candidates": []}
        mm = re.search(r"([A-Za-z]{3,9})-?(\d{4})", hdr[3])
        if mm:
            out["bill_month_text"] = {"value": mm.group(0), "raw_text": hdr[3],
                                      "bbox": [hdr[0], hdr[1], hdr[2]], "source_boxes": [list(hdr)],
                                      "confidence": None, "rule": "sparse:month",
                                      "observed": True, "candidates": []}
    return out
