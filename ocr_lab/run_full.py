...
def get_qc(mode):
    ...
    return (lambda p: uocr.parse_fields(uocr.transcribe(page_image(p)))), gemini_chain_ok
...
def main():
    ...
    qc_mode = None
    if "--qc" in argv:
        i = argv.index("--qc"); qc_mode = argv[i + 1]; del argv[i:i + 2]
    ...
    with open(os.path.join(OUT, "extract.jsonl"), "w", encoding="utf-8") as jf, \
        ...
        cw = csv.writer(cf); cw.writerow(["page"] + FLAT + ["chain_ok", "doc_type", "qc_resolved"])
    ...
    cov = {"pages": n, "kinds": kinds, "bill_pages": bills,
           "qc_mode": qc_mode, "qc_attempted": qc_attempted, "qc_resolved": qc_resolved,
