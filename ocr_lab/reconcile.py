...
def _check(name, expected, obs_field, add_recs):
    ...
    if obs_field is None or not obs_field.get("observed"):
        return {"name": name, "status": None, "reason": "no_observed_target",
                "expected": round(expected, 2), "observed": None}
...
def reconcile(fields, recs):
    ...
    checks = [
        ...
        _check("total",     tot, fields.get("total"),
    ...
    return {
        ...
        "computed": {"sub_total": sub, "total": tot, "net_bill": net, "net_payable": pay},
    }
...
def row_arithmetic(boxes, band=18):
    ...
    y_sub = next((b[0] for b in boxes if "subtotal" in b[3].lower().replace(" ", "")), 1e9)
    out = []
    for mb in money:
        if mb[0] >= y_sub - 2:
        ...
        if amt is None:
            continue
        sub_row = sorted([b for b in boxes if b is not mb and abs(b[0] - mb[0]) <= band
                          and b[1] < mb[1] - 3], key=lambda b: b[1])
        rate = qty = None
        # rate = first numeric box to the RIGHT of an 'Rs'/'Ps' marker (rate col).
        rs = next((b for b in sub_row if re.fullmatch(r"[RrPp]s\.?", b[3].strip())), None)
        if rs is not None:
            rcands = [b for b in sub_row if b[1] > rs[1] and parse_num(b[3])[0] is not None]
            if rcands:
        ...
        if units:
            u = units[0]
            qcands = [b for b in sub_row if b[1] < u[1] and parse_num(b[3])[0] is not None]
            if qcands:
